    ...
    def packet_handler(pkt):
        if pkt.haslayer(TCP) and pkt.haslayer(IP):
            try:
                ...
                conn['packets'].append(pkt_info)
                
                # Keep only last 50 packets per connection
                if len(conn['packets']) > 50:
                    conn['packets'] = conn['packets'][-50:]
                
                # Calculate RTT from packet analyzer
                analyzer.add_packet(
                    src_ip=ip_layer.src,
                    src_port=tcp_layer.sport,
                    dst_ip=ip_layer.dst,
